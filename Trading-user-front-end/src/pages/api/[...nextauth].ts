    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                if (OTP) {
                    try {
                        const user = await axios.post(`http://localhost:5000/trade-App/common/verifySecurityCode/${userId}`, {
                            OTP: parseInt(OTP)
                        })
                    ...
                    try {
                        const user: any = await axios.post(`http://localhost:5000/trade-App/user/login`, {
                            emailId,
                            password
                        })
